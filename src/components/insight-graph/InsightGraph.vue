<template>
  <div class="revenue-graph">
    ...
    <div
      ...
      <div :class="`label ${isCurrent(i) ? 'bold' : ''} ${isSmallOrDown ? 'txt-6' : 'txt-4'}`">
...
.revenue-graph {
  ...
  .column-wrapper {
    ...
    .column {
      ...
      &.grey { background-image: linear-gradient(map-get($colors, 'text-light'), map-get($colors, 'text-light')) }

      @media #{$small-and-down} { width: 50%; }
    }
  }
}
</style>
